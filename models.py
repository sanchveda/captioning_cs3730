...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim, decoder_dim, attention_dim, device='cpu', encoder_dim=2048):
    ...
    def forward(self, enc_out, enc_caps, len_cap):
        ...
        len_caps, idxs = len_cap.sort(dim=0, descending=True)
        ...
        predictions = torch.zeros(batch_size, max(len_decs), vocab_size).to(self.device)
        alphas = torch.zeros(batch_size, max(len_decs), num_pix).to(self.device)
        ...
        for t in range(max(len_decs)):
            ...
            indices = [l>t for l in len_decs]
            cbs = sum(indices) # cbs = current batch size
